...
function exportarPDF() {
    ...
    doc.setFontSize(14);
    doc.setTextColor(44, 40, 28);
    doc.text('CÂMARA MUNICIPAL DE CURITIBA', PW / 2, 13, { align: 'center' });

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(110, 90, 50);
    doc.text('Controle Orçamentário Legislativo', PW / 2, 19, { align: 'center' });
    ...
    doc.setFontSize(10);
    doc.setTextColor(44, 40, 28);
    doc.text(titulo.toUpperCase(), ML, 29);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(110, 90, 50);
    const metaRight = `${e.gab}   ·   ${e.mes}${e.responsavel ? '   ·   Resp.: ' + e.responsavel : ''}`;
    doc.text(metaRight, PW - MR, 29, { align: 'right' });
    ...
    doc.autoTable({
        ...
        styles: { font: 'helvetica', fontSize: 8, cellPadding: 3, textColor: [28, 27, 24], lineColor: [220, 210, 190], lineWidth: 0.2 },
        headStyles: { fillColor: [238, 228, 200], textColor: [44, 40, 28], fontStyle: 'bold', fontSize: 7.5 },
    ...
    doc.autoTable({
        ...
        didParseCell(data) {
            if (data.column.index === 1 && data.section === 'body') {
                data.cell.styles.textColor = data.cell.raw === 'Ocupado' ? [39, 120, 73] : [160, 114, 10];
    ...
    for (let i = 1; i <= total; i++) {
        ...
        doc.setFontSize(7);
        doc.setTextColor(110, 90, 50);
        doc.text('Câmara Municipal de Curitiba — Controle Orçamentário Legislativo', ML, PH - 5.5);
        doc.text(`Gerado em ${new Date().toLocaleDateString('pt-BR')}   ·   Página ${i} de ${total}`, PW - MR, PH - 5.5, { align: 'right' });
    }

    doc.save(`${e.gab} - ${e.mes}.pdf`.replace(/[/\\?%*:|"<>]/g, '-'));
}
